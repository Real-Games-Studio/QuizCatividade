...
public class ProjectSetup : MonoBehaviour
{
    ...
    void UpdateFolders()
    {
        ...
        else
        {
            ...
            File.Copy(Application.streamingAssetsPath + "/appconfig.json", "contents/appconfig.json", true);
            File.Copy(Application.streamingAssetsPath + "/gameconfig.json", "contents/gameconfig.json", true);
            File.Copy(Application.streamingAssetsPath + "/questions.json", "contents/questions.json", true);
    ...
    public static void LoadUIImage(string imageFileName, Image targetImage, Sprite preloadSprite = null)
    {
        ...
        if (preloadSprite == null)
        {
            if (File.Exists(imagePath))
            {
                ...
                Texture2D tex = new Texture2D(2, 2); // Tamanho n�o importa aqui
    ...
    public static IEnumerator LoadAudio(string mp3FileName, AudioSource audioSource, AudioClip preloadAudio = null)
    {
        ...
        if (preloadAudio == null)
        {
            ...
            if (File.Exists(path))
            {
                // Crie uma inst�ncia de UnityWebRequest
                ...
                {
                    Debug.LogError("Erro ao carregar o arquivo de �udio: " + www.error);
                }
                else
                {
                    // Configure o AudioSource para reproduzir o �udio carregado
            ...
            {
                Debug.LogError("Arquivo de �udio n�o encontrado: " + path);
            }
        }
        else
        {
    ...
    public static void LoadUnityVideo(string videoFileName, VideoPlayer videoPlayer) //n�o usar
    {
#if UNITY_EDITOR
        string projectPath = Application.dataPath.Replace("/Assets", "/contents");
#endif
        ...
        if (File.Exists(path))
        {
            // Configure o VideoPlayer para reproduzir o v�deo local
            videoPlayer.url = path;

            // Inicie a reprodu��o do v�deo
            videoPlayer.Play();
        }
        else
        {
            Debug.LogError("Arquivo de v�deo n�o encontrado: " + path);
        }
    }
    ...
    public static void LoadMaterial(string imageFileName, Renderer targetRenderer, Material preloadMaterial = null)
    {
        ...
        {
            Debug.LogError("A imagem n�o foi encontrada em StreamingAssets: " + imageFilePath);
    ...
    public static IEnumerator AudioNotFound(AudioSource audioSource)
    {
        ...
        if (File.Exists(path))
        {
            ...
            if (www.result == UnityWebRequest.Result.ConnectionError)
            {
                Debug.LogError("Erro ao carregar o arquivo de audio: " + www.error);
            }
            else
            {
                audioSource.clip = DownloadHandlerAudioClip.GetContent(www);
        ...
        {
            Debug.LogError("Arquivo de audio nao encontrado: " + path);
        }
    }
